...
function renderSalesChart(data) {
    ...
    try {
        ...
        // 销售额Y轴的最大值（确保有足够的刻度范围）
        let salesYAxisMax = Math.ceil(maxSales * 1.2);
        // 确保销售额Y轴最大值至少为100，以避免刻度过小
        if (maxSales > 0 && salesYAxisMax < 100) {
            salesYAxisMax = 100;
        }
        
        // 订单数量Y轴的最大值（确保有足够的刻度范围）
        let ordersYAxisMax = Math.ceil(maxOrders * 1.2);
        // 确保订单数量Y轴最大值至少为10，以避免刻度过小
        if (maxOrders > 0 && ordersYAxisMax < 10) {
            ordersYAxisMax = 10;
        }
        
        // 销售额Y轴的刻度步长（确保至少有5个刻度）
        const salesYAxisStepSize = Math.max(1, Math.ceil(salesYAxisMax / 5));
        // 订单数量Y轴的刻度步长（确保至少有5个刻度）
