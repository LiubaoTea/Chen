...
window.viewOrderDetail = async function(orderId) {
    try {
        ...
        if (orderDetail.status === 'delivered') {
            orderDetailHTML += `
                    <button class="review-now-btn" onclick="window.showReviewForm('${orderDetail.order_id}')">立即评价</button>
            `;
        }
...
// 显示商品评价表单
window.showReviewForm = async function(orderId) {
    try {
        const token = localStorage.getItem('userToken');
        const response = await fetch(`${API_BASE_URL}/api/orders/${orderId}`, {
            headers: {
        ...
        let reviewFormHTML = `
            <div class="review-form-container">
                <h3>商品评价</h3>
                <p class="review-tip">您的评价将帮助其他顾客做出更好的购买决定</p>
                <form id="productReviewForm">
                    <input type="hidden" id="orderIdInput" value="${orderId}">
        `;
                    ...
                    <div class="review-product-item">
                        <div class="review-product-info">
                            ...
                            <div class="review-product-details">
                                ...
                                <p>单价: ¥${item.unit_price.toFixed(2)} × ${item.quantity}</p>
                            </div>
                        </div>
                        ...
                        <div class="review-rating-container">
                            ...
                            <div class="rating-stars" data-product-id="${item.product_id}">
                                ...
                                <input type="hidden" name="rating_${item.product_id}" id="rating_${item.product_id}" value="5">
                            </div>
                        </div>
                        
                        <div class="review-content-container">
                            <p>评价内容:</p>
                            <textarea name="content_${item.product_id}" id="content_${item.product_id}" rows="3" placeholder="请分享您对商品的使用体验和感受..."></textarea>
                        </div>
                    </div>
                `;
            });
        ...
        reviewFormHTML += `
                    <div class="review-form-actions">
                        <button type="button" class="cancel-review-btn" onclick="closeReviewModal()">取消</button>
                        <button type="button" class="submit-review-btn" onclick="submitProductReview()">提交评价</button>
                    </div>
                </form>
            </div>
        `;
        ...
        // 显示评价表单模态框
        showSettingsModal(reviewFormHTML);
        ...
        console.error('加载评价表单失败:', error);
        showErrorMessage('加载评价表单失败，请稍后重试');
    }
};
    ...
    try {
        ...
        if (orderDetail.items && orderDetail.items.length > 0) {
            orderDetail.items.forEach(item => {
                ...
                const content = document.getElementById(`content_${productId}`).value.trim();
        ...
        if (hasEmptyContent) {
            showErrorMessage('请填写所有商品的评价内容');
            return;
        }
        
        if (reviews.length === 0) {
            showErrorMessage('评价提交失败，请填写评价内容');
            return;
        }
        ...
        for (const review of reviews) {
            ...
            if (!reviewResponse.ok) {
                ...
                throw new Error(errorData.error || '评价提交失败');
            }
        }
